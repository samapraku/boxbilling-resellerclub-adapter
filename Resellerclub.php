...
class Registrar_Adapter_Resellerclub extends Registrar_AdapterAbstract
{
    ...
    public function modifyDomainContact(Registrar_Domain $domain, $sixtyDayLockoOptout = null)
    {
        ...
        if (!is_null($sixtyDayLockoOptout)) {
            $optional_params['sixty-day-lock-optout'] = $sixtyDayLockoOptout;
        }
    ...
    private function _getContact($contact, $customer_id, $type = 'Contact', array $data = null)
    {
        try {
            $params = array(
                'customer-id'   => $customer_id,
    ...
    public function searchDomains($data = []) {
        $page_num = $data['page-no'] ?? 1;
        $num_of_records = $data['page-no'] ?? 20;
        $status = $data['status'] ?? [];
        try{
            $params = [
                'no-of-records' => $page_num,
                'page-no'   => $num_of_records,
                'status'    => $status,
            ];
            
            $domains = $this->_makeRequest('domains/search', $params, 'GET', 'json');
            $this->getLog()->info(json_encode($domains));
            }
             catch(Registrar_Exception $e) {
            $this->getLog()->info($e->getMessage());
        }
